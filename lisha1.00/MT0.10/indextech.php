...
<html>
	<head>
		...
		<link rel="stylesheet" href="css/doc/index.css" type="text/css"> <!-- * load custom page style * -->
		<link rel="stylesheet" href="css/doc/tiny_details.css" type="text/css"> <!-- * load custom tiny page style * -->
		...
		<script type="text/javascript" src="js/doc/tiny/tiny_mce.js"></script> <!-- TinyMCE for documentation -->
		...
		<script  type="text/javascript" src="js/doc/index.js"></script> <!-- Specific javascript of current page -->
		<script type="text/javascript">
		...
		<?php 
			 ...
			 * MagicTree definition of each tree in page
			 ====================================================================*/	
			include ('./includes/MTSetupDoc/define/MT_tech_doc.php');
			$_SESSION[$ssid]['MT'][$mt_tech_id]->generate_html_header();
			/*===================================================================*/
	...
	<body onload="init_load('<?php echo $_SESSION[$ssid]['MT']['langue_TinyMCE'];?>','<?php echo $id_page;?>');<?php echo 'MagicTree_'.$mt_tech_id.'()'?>;">
		<!-- ================================================== MSGBOX ================================================= -->	
		<div id="iknow_msgbox_background"></div>
		<div id="iknow_msgbox_conteneur" style="display:none;"></div>
		<!-- ===============================================  END MSGBOX ==============================================  -->
		<?php echo '<div id="'.$mt_tech_id.'" style="position: absolute; top:0; left:0; height:100%; width:500px; font-size: 0.9em; box-shadow: 5px 2px 8px #333;"></div>';?>
		<div id="gauche" style="float: left; width:30%; height:100%; display:block;"></div>
		<div id="main_details" style="float: left;">
			<div id="headdetails" class="headdetails" <?php if(!$_SESSION[$ssid]["edit_mode"]) echo 'onclick=\'MainTimer.add_event(1,"reduce_tool_bar()");\'';echo 'MagicTree_'.$mt_tech_id.'();';?> style="height: 0px;">
				<?php
				echo '<div class="welcome">'.$_SESSION[$ssid]['page_text'][1]['MT'].'</div>';
				if($_SESSION[$ssid]["edit_mode"])
				{
				echo '
				<div class="boutton_security_off" onClick="user_tree_mode();" onmouseout="lib_hover(\'\')" onmouseover="lib_hover(\''.js_protect($_SESSION[$ssid]['page_text'][5]['LT']).'\')"></div>
				<div id="tool_button_edit" class="tool_button_edit">
				<div id="boutton_edit" class="boutton_edit" onClick="show_tiny(\''.$_SESSION[$ssid]['langue'].'\')" onmouseout="lib_hover(\'\')" onmouseover="lib_hover(\''.js_protect($_SESSION[$ssid]['page_text'][8]['LT']).'\')"></div>
					<div id="boutton_back" class="boutton_back" onClick="html_detail_display(\'ikdoc\')" onmouseout="lib_hover(\'\')" onmouseover="lib_hover(\''.js_protect($_SESSION[$ssid]['page_text'][9]['LT']).'\')"></div>
					</div>
					';	
				}
				else
				{
					echo '
					<div class="boutton_security" onmouseout="lib_hover(\'\')" onmouseover="lib_hover(\''.js_protect($_SESSION[$ssid]['page_text'][2]['LT']).'\')" onClick="user_tree_mode();"></div>
					';
				}
				?>
			</div>
